...
// DOM Elements - Consolidate similar elements
...
const currencyInputs = transactionForm.querySelectorAll(
...
const transactionErrorMessage = document.getElementById(
...
function renderMembersList() {
  ...
  party.members.forEach((member, index) => {
    ...
    memberCard.className =
      "p-4 bg-surfaceContent rounded-lg shadow-md flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4";
    memberCard.innerHTML = `
                <div>
                    <h4 class="text-xl font-semibold text-primary-light mb-1">${
                      member.name
                    }</h4>
                    <div class="flex flex-wrap gap-x-4 gap-y-1 text-sm text-textSecondary">
                        ${CURRENCIES.map(
                          (c) =>
                            `<span>${c.toUpperCase()}: <strong class="text-textPrimary">${
                              member.funds[c] || 0
                            }</strong></span>`
                        ).join("")}
                    </div>
...
function renderTransactionHistory() {
  ...
  if (!party || !party.transactionHistory) {
    historySection.innerHTML =
      '<p class="text-center text-textSecondary italic">No transactions yet.</p>';
    return;
  }
                ...
                ${party.transactionHistory
                  .map(
                    ...
                    <div class="p-4 bg-surfaceContent rounded-lg shadow">
                        <div class="text-sm text-textSecondary">${new Date(
                          tx.timestamp
                        ).toLocaleString()}</div>
                        ...
                        <div class="text-sm text-textSecondary">
                            ...
                            ${
                              tx.amount
                                ? `Amount: ${tx.amount.pp}pp ${tx.amount.gp}gp ${tx.amount.ep}ep ${tx.amount.sp}sp ${tx.amount.cp}cp`
                                : ""
                            }
                        </div>
                    </div>
  ...
  try {
    switch (type) {
      ...
      case "party_withdraw":
        ...
        if (
          isNaN(targetMemberIndex) ||
          targetMemberIndex < 0 ||
          targetMemberIndex >= party.members.length
        ) {
      ...
      case "member_withdraw":
        ...
        if (subtractFunds(memberGiving.funds, transactionAmount)) {
          ...
          throw new Error(
            `Insufficient funds in ${memberGiving.name}'s wallet.`
          );
        }
        break;
      case "member_to_member":
        ...
        if (
          isNaN(toMemberIdx) ||
          toMemberIdx < 0 ||
          toMemberIdx >= party.members.length ||
          toMemberIdx === memberIndex
        ) {
    ...
    if (success) {
      transactionHistory.addTransaction({
        type: type,
        from: type.includes("member")
          ? party.members[memberIndex].name
          : "Party Treasury",
        to: type.includes("member")
          ? type === "member_to_member"
            ? party.members[toMemberIdx].name
            : "Party Treasury"
          : party.members[memberIndex].name,
      ...
      cloudStorage
        ...
        .then((data) => {
          if (data) {
            ...
            showMessage(
              `Party "${name}" created! Add members and manage funds.`
            );
          }
          renderAll();
        })
...
function handleAddMember() {
  ...
  if (
    party.members.find((m) => m.name.toLowerCase() === memberName.toLowerCase())
  ) {
...
function handleRemoveMember(event) {
  ...
  if (
    confirm(
      `Are you sure you want to remove ${member.name}? Their funds will be lost if not transferred first.`
    )
  ) {
...
function init() {
  auth.getCurrentUser().then((user) => {
    if (user) {
      cloudStorage
        .loadPartyData(user.lastPartyId)
        .then((data) => {
          if (data) {
            party = data;
            ...
            party = null;
            showMessage(
              "Create a new party or type an existing party name to load.",
              false
            );
            renderAll();
          }
  ...
  depositPartyButton.addEventListener("click", () =>
    openTransactionModal("party_deposit")
  );
  withdrawPartyButton.addEventListener("click", () =>
    openTransactionModal("party_withdraw")
  );
